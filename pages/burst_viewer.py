...
if search_query:
    ...
    search_query = (
        search_query if search_query.startswith("GRB") else "GRB" + search_query
    )
    ...
    flares = tab_events[tab_events["event_type"] == "flare"]
    flares = flares[flares["GRBname"].str.upper() == search_query]

    pulses = tab_events[tab_events["event_type"] == "pulse"]
    pulses = pulses[pulses["GRBname"].str.upper() == search_query]
    ...
    if not all([afterglow.empty, flares.empty, pulses.empty]):
        ...
        summary_data_left = {
            ...
            "Redshift": get_table_value(
                afterglow, "redshift", error="redshift_err", format="%.2g"
            ),
            "Afterglow Fluence (erg cm$^{-2}$)": get_converted_fluence(
                afterglow, "fluence", "conversion"
            ),
        }
        ...
        st.markdown(
            ":grey[:small[See [About LAFF](/about_laff) for a description of the model parameters, and the fitting procedure.]]"
        )
        ...
        bat_image_path = os.path.join(
            dataset_path, "figures/bat", f"{search_query}.png"
        )

        bat_plot, bat_table = st.columns(
            [0.6, 0.4], border=True, vertical_alignment="center"
        )
        ...
        with bat_table:
            ...
            summary_data = {
                ...
                "Conversion Reduced Chi-Square": get_table_value(
                    pulses, "bat_conversion_rchisq"
                ),
                "Total fluence in pulses": ("%.3g" % total_pulse_fluence)
                if (bat_conversion != "-" and not pd.isna(bat_conversion))
                else "-",
            }
        ...
        if len(pulses):
            pulse_table = pd.DataFrame(
                {
                    "Pulse": get_table_multiple_values(
                        pulses, "pulse_num", format="%d"
                    ),
                    ...
                    "Peak Flux (erg/cm^2/s^1)": get_table_multiple_values(
                        pulses, "peak_flux"
                    ),
                    "Isotropic Energy (erg)": get_table_multiple_values(
                        pulses, "e_iso"
                    ),
            ...
            pulse_table_model = pd.DataFrame(
                {
                    "Pulse Number": get_table_multiple_values(
                        pulses, "pulse_num", format="%d"
                    ),
        ...
        if afterglow.empty and flares.empty:
            ...
            xrt_image_path = os.path.join(
                dataset_path, "figures/xrt", f"{search_query}.png"
            )

            xrt_plot, xrt_table = st.columns(
                [0.6, 0.4], border=True, vertical_alignment="center"
            )
            ...
            with xrt_table:
                ...
                total_flare_fluence = (
                    "%.3g" % sum([x for x in flares["fluence"]])
                    if len(flares["fluence"])
                    else "-"
                )
            ...
            if len(flares):
                ...
                flare_table = pd.DataFrame(
                    {
                        "Flares": get_table_multiple_values(
                            flares, "flarenum", format="%d"
                        ),
                        ...
                        "Underlying Index": get_table_multiple_values(
                            flares, "underlying_index"
                        ),
                        "Fluence (erg/cm^2)": get_table_multiple_values(
                            flares, "fluence"
                        ),
                        "Peak Flux (erg/cm^2/s^1)": get_table_multiple_values(
                            flares, "peak_flux"
                ...
                flare_table_model = pd.DataFrame(
                    {
                        "Flare Number": get_table_multiple_values(
                            flares, "flarenum", format="%d"
                        ),
